...
{
    $query = Order::with('user', 'payment', 'items.product', 'booking.table')
    ...
    public function previewReceipt($id)
    {
        ...
        // Kita load relasi: items (detail pesanan), payment (info bayar), booking (cek meja)
        $order = Order::with(['items', 'payment', 'booking.table'])->findOrFail($id);

        // Tentukan tipe pesanan (logika dari snippet laporan Anda)
        $orderType = ($order->booking && $order->booking->table) ? 'Dine In' : 'Take Away';
        
        // Siapkan data customer
        $customer = [
            'name' => $order->customer_name,
            // Jika di tabel orders tidak ada alamat, bisa dikosongkan atau ambil dari relasi lain
            'address' => '-', 
        ];

        return view('orders.show', compact('order', 'orderType', 'customer'));
    }
    ...
    public function export($id)
    {
        ...
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('orders.show', compact('order'))
            ->setPaper([0, 0, 172, 300], 'portrait'); 
