...
<head>
    ...
    <title>Struk #{{ $order->payment->transaction_id ?? $order->id }}</title>
...
<body>
    ...
    <div class="app-container screen-only">
        ...
        <div class="receipt-card {{ $order->payment->status == 'paid' ? 'status-paid' : 'status-pending' }}">
            ...
            <div class="receipt-body">
                <div class="info-row">
                    ...
                    <span class="info-val">{{ $order->payment->transaction_id ?? 'TRX-'.$order->id }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Waktu (WIB)</span>
                    <span class="info-val">{{ $order->created_at->setTimezone('Asia/Jakarta')->format('d M, H:i') }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Tipe</span>
                    <span class="info-val">
                        @if($order->booking)
                            <span class="badge bg-primary rounded-pill">Meja {{ $order->booking->table->table_number }}</span>
                        @else
                ...
                <div class="item-list">
                    ...
                    <div class="item-row">
                        <div style="flex: 1;">
                            ...
                            <div class="item-meta">{{ $item->quantity }} x {{ number_format($item->price, 0, ',', '.') }}</div>
                        </div>
                ...
                <div class="total-box">
                    ...
                    <div class="d-flex justify-content-between align-items-center text-muted small">
                        <span>Metode: {{ strtoupper($order->payment->payment_method) }}</span>
                ...
                <div class="text-center mt-3 text-muted" style="font-size: 0.8rem;">
                    Kasir: {{ Auth::user()->username ?? 'Admin' }}
                </div>
            </div>
        ...
        <div class="action-area">
            ...
            <div class="btn-group-row">
                ...
                <a href="{{ route('orders.history') }}" class="btn-outline">
    ...
    <div class="thermal-print-area">
        ...
        <table class="thermal-table">
            <tr><td>No</td><td class="thermal-right">{{ $order->payment->transaction_id ?? $order->id }}</td></tr>
            <tr><td>Tgl</td><td class="thermal-right">{{ $order->created_at->setTimezone('Asia/Jakarta')->format('d/m/y H:i') }}</td></tr>
            <tr><td>Cust</td><td class="thermal-right">{{ substr($order->customer_name, 0, 15) }}</td></tr>
            <tr>
                <td>Tipe</td>
                <td class="thermal-right">{{ $order->booking ? 'Meja '.$order->booking->table->table_number : 'Take Away' }}</td>
            </tr>
        </table>
        ...
        <div style="margin-bottom: 2px;">
            <div class="thermal-bold">{{ $item->product->name ?? 'Item' }}</div>
        ...
        <table class="thermal-table">
            <tr><td>Bayar</td><td class="thermal-right">{{ strtoupper($order->payment->payment_method) }}</td></tr>
            <tr>
                <td>Status</td>
                <td class="thermal-right">{{ $order->payment->status == 'paid' ? 'LUNAS' : 'PENDING' }}</td>
            </tr>
            <tr><td>Kasir</td><td class="thermal-right">{{ Auth::user()->username ?? '-' }}</td></tr>
        </table>
    ...
    <script>
        ...
        function printQZ() {
            qz.printers.getDefault().then(printer => {
                ...
                let itemsData = [];
                @foreach ($order->items as $item)
                    itemsData.push('{{ $item->product->name ?? "Item" }}\n');
                    itemsData.push('  {{ $item->quantity }} x ' + fmt({{ $item->price }}) + '     Rp ' + fmt({{ $item->quantity * $item->price }}) + '\n');
                @endforeach

                // LOGIKA JS QZ TRAY: PAID = LUNAS, LAINNYA = PENDING
                let statusText = "{{ $order->payment->status == 'paid' ? 'LUNAS' : 'PENDING' }}";
                ...
                let data = [
                    ...
                    '--------------------------------\n',
                    '\x1B\x61\x00', // Left
                    'No Trx : {{ $order->payment->transaction_id ?? "-" }}\n',
                    'Tgl    : {{ $order->created_at->setTimezone("Asia/Jakarta")->format("d/m/y H:i") }}\n',
                    'Cust   : {{ $order->customer_name }}\n',
                    'Tipe   : {{ $order->booking ? "Meja ".$order->booking->table->table_number : "Take Away" }}\n',
                    '--------------------------------\n',
                    ...itemsData,
                    '--------------------------------\n',
                    '\x1B\x61\x02', // Right
                    '\x1B\x45\x01', // Bold
                    'TOTAL: Rp ' + fmt({{ $order->total_amount }}) + '\n',
                    '\x1B\x45\x00', // No Bold
                    '--------------------------------\n',
                    '\x1B\x61\x00', // Left
                    'Bayar  : {{ strtoupper($order->payment->payment_method) }}\n',
                    'Status : ' + statusText + '\n',
